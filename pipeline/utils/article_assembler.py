...
def assemble_article(
    ...
    # 7a. Strip any remaining before-after references. The component is
    #     banned (see writer_backstory.txt and qa_gate.py check 19).
    #     Covers three cases:
    #       - {{< before-after ... >}} shortcodes the writer emitted
    #       - ![Before After visualization](...stat_card.png) leftovers
    #         from old data_extractor runs
    #       - Raw BEFORE_AFTER: markers that _convert_visual_markers
    #         no longer converts
    body = _strip_banned_visuals(body)
...
def _convert_visual_markers(body: str) -> str:
    ...
    Markers generated by the pipeline (TIMELINE:, PROCESS_FLOW:, etc.)
    are converted to Hugo shortcode format which the remark plugin
    transforms into React components.

    Note: BEFORE_AFTER: marker conversion has been removed. The
    {{< before-after >}} component is banned (low signal-to-noise, text
    clipping — see writer_backstory.txt BEFORE / AFTER CARD section).
    Any leftover BEFORE_AFTER: markers in the writer output are instead
    stripped by _strip_banned_visuals() below so they don't reach the
    published article as raw text.
    """
